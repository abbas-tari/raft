...
def update_frame(frame_number):
    ...
    leader_index = (frame_number // 20) % len(distributed_formation_objects)
    ...
    if leader_node.failed:
        new_leader_index = (leader_index + 1) % len(distributed_formation_objects)
        leader_node = distributed_formation_objects[new_leader_index]
...
animation = FuncAnimation(fig, update_frame, frames=142, interval=2, blit=False, repeat=False)
plt.show()
