...
class DistributedFormation(SyncObj):
    def __init__(self, selfNodeAddr, otherNodeAddrs):
        ...
        self.agents = [
            (random.uniform(-4, 5), random.uniform(-.2, 3)) for _ in range(N_AGENTS)
...
def plot_agents_positions(formation_objects, ax):
    ...
    for i, formation_obj in enumerate(formation_objects):
        ...
        if formation_obj.leader_id == i:
            ax.scatter(x, y, label=f'Agent {i} (Leader)', color='blue')
        else:
            ax.scatter(x, y, label=f'Agent {i}', color='orange')
...
def update_frame(frame_number):
    ...
    # Simulate leader failure at frame 15 and log the event
    if frame_number == 15:
    ...
    if frame_number == 16:
        ...
        for formation_obj in distributed_formation_objects:
            ...
            formation_obj.leader_offset = 0  # Update the leader_offset
...
animation = FuncAnimation(fig, update_frame, frames=121, interval=1, blit=False, repeat=False)
plt.show()
